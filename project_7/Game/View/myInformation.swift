...
class myInformation : UIView ,MyModelDelegate{

    var model: MyModel? {
        didSet{
            Blood?.frame.size.width = CGFloat(model!.MyBlood*205/100)
            DyingBlood?.frame.size.width = CGFloat(model!.MyDyingBlood*205/100)
            Armor?.frame.size.width = CGFloat(model!.MyArmor*175/100)
            UIView.animate(withDuration: 0.3, animations: {
                self.Blood_2?.frame.size.width = CGFloat(self.model!.MyBlood*205/100)
                self.DyingBlood_2?.frame.size.width = CGFloat(self.model!.MyDyingBlood*205/100)
                self.Armor_2?.frame.size.width = CGFloat(self.model!.MyArmor*175/100)
            })
            MyName?.text = model!.MyName
            model?.delegate = self
        }
    }
    ...
    func didUpdate(Model: MyModel) {
        ...
        Armor?.frame.size.width = CGFloat(Model.MyArmor*175/100)
        UIView.animate(withDuration: 0.3, animations: {
            self.Blood_2?.frame.size.width = CGFloat(Model.MyBlood*205/100)
            self.DyingBlood_2?.frame.size.width = CGFloat(Model.MyDyingBlood*205/100)
            self.Armor_2?.frame.size.width = CGFloat(Model.MyArmor*175/100)
        })
        MyName?.text = Model.MyName
    }
}
