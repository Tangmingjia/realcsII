...
class Bag : UIView ,MyBagModelDelegate{
    
    var model: MyBagModel? {
        didSet{
            myBlood?.text = "血量：\(model!.MyBlood)"
            myArmor?.text = "护甲：\(model!.MyArmor)"
            nickName?.text = model!.MyName
            IconView?.kf.setImage(with: URL(string: (model!.MyIcon.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)), placeholder: UIImage(named: "logo.png"))
            KillNum?.text = "击杀：\(model!.MyKill)"
            Damage?.text = "伤害：\(model!.MyDamage)"
            model?.delegate = self
        }
    }
